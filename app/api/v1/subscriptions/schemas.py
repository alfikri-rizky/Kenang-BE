...
class PlanLimits(BaseModel):
    ...
    max_circles: Optional[int] = Field(None, description="Maksimum lingkaran (None = unlimited)")
    max_photos_per_circle: Optional[int] = Field(None, description="Maksimum foto per lingkaran")
    ...
    max_members_per_circle: Optional[int] = Field(None, description="Maksimum anggota per lingkaran")
...
class PlanResponse(BaseModel):
    ...
    billing_cycle: str = Field(..., description="Siklus pembayaran: monthly, yearly, one_time, lifetime")
    ...
    recommended_for: List[str] = Field(default_factory=list, description="Rekomendasi untuk siapa paket ini")
...
class CheckoutResponse(BaseModel):
    ...
    redirect_url: str = Field(..., description="URL untuk redirect ke halaman pembayaran Midtrans")
...
class SubscriptionResponse(BaseModel):
    ...
    current_period_start: Optional[datetime] = Field(None, description="Tanggal mulai periode subscription")
    current_period_end: Optional[datetime] = Field(None, description="Tanggal akhir periode subscription")
    cancelled_at: Optional[datetime] = Field(None, description="Tanggal dibatalkan (jika ada)")
...
class PaymentResponse(BaseModel):
    ...
    payment_provider: Optional[str] = Field(None, description="Provider pembayaran")
    payment_provider_transaction_id: Optional[str] = Field(None, description="Transaction ID dari provider")
    status: str = Field(..., description="Status: pending, success, failed, refunded, expired")
    completed_at: Optional[datetime] = Field(None, description="Tanggal pembayaran selesai")
    created_at: datetime = Field(..., description="Tanggal dibuat")
...
class MidtransWebhookPayload(BaseModel):
    ...
    payment_type: str = Field(..., description="Tipe pembayaran (gopay, bank_transfer, dll)")
    ...
    va_numbers: Optional[List[Dict]] = Field(None, description="VA numbers (for bank transfer)")
